...
We should be able to random samples from the same overall distribution and see the same pattern most of the time. Here are 10 random re-samples from the same data.
```{r echo = T, results = 'false', message=FALSE, warning = F}
#df[sample(nrow(df), 3), ]
for (i in 1:10) {
...
p1 <- ggplot(df_long, aes(Time)) +
  geom_density() +
  theme_classic() +
  labs(title = "Distribution of Data for All Conditions")

p2 <- ggplot(df_long, aes(Time, fill = Condition)) +
  geom_density(alpha = 0.5) +
  theme_classic() +
  labs(title = "Distribution of Data Modelled by Condition")
